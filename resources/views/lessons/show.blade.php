...
@section('content')
    <article class="stack lesson-page">
        <header class="page-heading"><div><p class="lesson-breadcrumb"><a href="{{ route('courses.show', $lesson->course) }}">{{ $lesson->course->title }}</a>@if($lesson->module)<span aria-hidden="true">/</span> {{ $lesson->module->title }}@endif<span aria-hidden="true">/</span> Lesson</p><h1>{{ $lesson->title }}</h1><p>{{ $lesson->summary }}</p></div></header>
        ...
        @if(auth()->user()->isUser())
            <form method="POST" action="{{ route('lessons.complete', $lesson) }}" class="lesson-complete">
                @csrf
                <button type="submit">{{ $isCompleted ? 'Lesson completed' : 'Mark lesson complete' }} <span aria-hidden="true">✓</span></button>
            </form>
        @endif
